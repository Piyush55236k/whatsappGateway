...
export default function WebhooksPage() {
    ...
    const generateNewApiKey = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setShowApiKey(true); // show once, as it cannot be retrieved later
                toast.success("New API key generated. Copy it now — it will not be displayed again!", { duration: 8000 });
            }
        } catch (error) {
    ...
    const handleTestWebhook = async (webhook: WebhookConfig) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data?.ok) {
                toast.success(data.message || "Webhook test succeeded!");
            } else {
            ...
            <Card>
                ...
                <CardContent>
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                        <div className="flex-1 bg-slate-100 rounded-md p-2 sm:p-3 font-mono text-xs sm:text-sm overflow-x-auto">
                            {apiKey ? (
                                ...
                                <span className="text-muted-foreground">•••••••• (key is hidden — only displayed once upon creation)</span>
                            ) : (
                    ...
                    {!apiKey && apiKeySet && apiKeyHidden && (
                        <p className="text-xs text-muted-foreground mt-2">
                            For security reasons, your API key is encrypted and cannot be displayed again. Click <b>Regenerate</b> if you lost or forgot your key.
                        </p>
                    )}
                </CardContent>
