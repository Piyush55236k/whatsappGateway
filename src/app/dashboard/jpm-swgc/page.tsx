...
export default function JpmSwgcPage() {
    ...
    const handleSend = async () => {
        ...
        const confirmed = confirm(
            scope === "ALL"
                ? `Send status to ALL groups (${groups.length} groups)? This action is resource-intensive.`
                : `Send status to ${targets.length} selected group(s)?`
        );
        if (!confirmed) return;
        ...
        try {
            ...
            const data = await res.json();
            if (data.status) {
                toast.success(`Dispatch started for ${data.data?.total || 0} group(s). Check server logs for details.`);
            ...
            <div className="max-w-4xl space-y-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Bulk Status Group Channel — broadcast message/photo/video as group status to all or selected groups.
                    </p>
                </div>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-2">
                            ...
                            <Textarea
                                ...
                                placeholder="Write a message to send as group status..."
                                rows={4}
                            />
                        </div>
