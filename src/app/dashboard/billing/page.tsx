...
export default function BillingPage() {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (status === "PAID") {
                    ...
                    setPolling(false);
                    toast.success(`Payment successful! Plan ${checkout.planName} is now active 🎉`);
                    setCheckout(null);
                    loadUsage();
    ...
    const handleBuy = async (plan: PlanId) => {
        ...
        if (plan === "FREE") return;
        if (cfg.price === null) {
            toast.info("Enterprise plan is custom. Please contact support or sales.");
            return;
        }
        setBuying(plan);
        try {
            ...
            if (!json.data.qrString && !json.data.qrImageUrl) {
                toast.error("Gateway did not return QR code. Check payment configuration.");
                return;
            }
                    ...
                    <CardContent className="flex flex-col items-center gap-4">
                        <div className="bg-white p-4 rounded-2xl">
                            {checkout.qrString ? (
                                ...
                                <img src={checkout.qrImageUrl} alt="QR Code" width={240} height={240} />
                            ) : null}
                        </div>
                        <div className="text-center">
                            ...
                            {checkout.totalAmount && checkout.totalAmount !== checkout.amount && (
                                <p className="text-xs text-muted-foreground">
                                    Plan price {formatIDR(checkout.amount)} + fee/unique code
                                </p>
                            )}
