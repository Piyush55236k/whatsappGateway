...
export async function PricingCards({ ctaHref = "/dashboard/billing" }: { ctaHref?: string }) {
    ...
    return (
        <div className="relative">
                ...
                return (
                    ...
                    >
                        ...
                        <Link href={isCustom ? "/docs" : ctaHref} className="mt-auto">
                            ...
                            >
                                {isFree ? "Get Started Free" : isCustom ? "Contact Us" : `Select ${plan.name}`}
                            </Button>
                        </Link>
                    </div>
                );
            ...
            <p className="md:hidden text-center text-xs text-muted-foreground mt-1">← swipe to view all plans →</p>
        </div>
    );
}
